...
test.describe('Epic 1: Account Management & Creation', () => {
    ...
    test('1.1 Create New Account Successfully (Happy Path)', async ({ page }) => {
        ...
        await expect(historyRow).toContainText('DEPOSIT');
        await expect(historyRow).toContainText('$10,000.00');
    });
...
test.describe('Epic 2: Cash Management', () => {
    ...
    test('2.1 Successful Deposit and Balance Update', async ({ page }) => {
        ...
        await expect(page.getByLabel('Status').nth(1)).toHaveValue('Success: $2,000.00 deposited. Your new cash balance is $12,000.00.');
