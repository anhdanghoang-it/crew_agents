...
*   **Net Deposits:** The total amount of money deposited by the user minus the total amount of money withdrawn. This represents the user's total capital contribution.
    *   *Formula:* `Sum(All DEPOSIT Transactions) - Sum(All WITHDRAWAL Transactions)`
*   **Total Portfolio Value:** The current total worth of the user's account. It includes both the cash they hold and the current market value of all their shares.
    *   *Formula:* `Current Cash Balance + Sum(Quantity of each share * Current Price of each share)`
*   **Profit / Loss (P/L):** The overall gain or loss of the account relative to the user's net capital contribution.
...
*   **AC-1: Successful Account Creation (Happy Path)**
    ...
    *   **Then** the system creates a new user account for 'trader123'.
    *   **And** the initial cash balance is set to $10,000.00.
    *   **And** the UI displays a success message: "Success: Account 'trader123' created with an initial deposit of $10,000.00."
    *   **And** a 'DEPOSIT' transaction for $10,000 is recorded in the user's transaction history.
...
*   **Wireframe/Mockup:**
    ...
    +-------------------------------------------+
    ```
*   **Accessibility:** All input fields should have clear labels. The "Create Account" button should be reachable via keyboard navigation (Tab key). Status messages should be readable by screen readers.

#### **Non-Functional Requirements**
*   **Performance:** Account creation and UI feedback should complete within 500ms.
*   **Security:** Passwords must be securely hashed (e.g., using bcrypt) before storage. All user inputs must be sanitized to prevent injection attacks.
...
*   **Wireframe/Mockup:**
    ...
    +-------------------------------------------+
    ```
*   **Accessibility:** Buttons should have descriptive text and be keyboard accessible. Status messages must be announced by screen readers.

#### **Non-Functional Requirements**
*   **Performance:** Transaction processing and UI update should occur within 300ms.
*   **Data Integrity:** All cash transactions must be atomic to prevent race conditions during concurrent operations.
...
*   **Wireframe/Mockup:**
    ...
    +-------------------------------------------+
    ```
*   **Accessibility:** All form controls should be keyboard navigable and have associated labels for screen readers.

#### **Non-Functional Requirements**
*   **Performance:** Trade execution, including the call to `get_share_price` and UI update, should complete within 1 second.
*   **Reliability:** The system must gracefully handle API failures from `get_share_price` and display a user-friendly error message, such as "Error: Could not retrieve share price. Please try again later."
...
*   **Priority:** High
*   **Dependencies:** US-001. Data is generated by **US-002 (Cash Management)** and **US-003 (Trading)**.
...
*   **AC-1: View Portfolio Summary with Correct Calculations**
    ...
    *   **And** my total deposits are $12,000 and total withdrawals are $2,000 (resulting in a Net Deposit of $10,000).
    *   **And** the current price of 'AAPL' is $150 and 'TSLA' is $300.
    *   **When** I navigate to the 'Portfolio' view.
    *   **Then** the UI displays my current holdings in a table: "AAPL: 10 shares", "TSLA: 5 shares".
    ...
    *   **And** it calculates and displays the Total Profit/Loss using the formula `Total Portfolio Value - Net Deposits`: `$8,000 - $10,000 = -$2,000`.
...
*   **AC-2: View Transaction History**
    ...
    *   **Then** the UI displays a table of all transactions in reverse chronological order (most recent first).
...
*   **AC-3: View Empty Portfolio and History**
    ...
    *   **Then** the share holdings section is empty or shows a message like "You do not own any shares."
    *   **And** the Total Portfolio Value is equal to my initial deposit (which is my cash balance).
    *   **And** the Profit/Loss is $0.
    *   **And** when I view my history, the table shows only the initial 'DEPOSIT' transaction.
...
*   **Gradio Components for "Portfolio" Tab:**
    ...
    *   `gr.Button("Refresh")` to get the latest prices and recalculate all values.
...
*   **Wireframe/Mockup ("Portfolio" Tab):**
    ...
    +-------------------------------------------+
    ```
*   **Accessibility:** All displayed data in `gr.Textbox` should be associated with its label for screen readers. `gr.Dataframe` tables should be navigable and readable.
